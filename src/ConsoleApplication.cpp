...
void ConsoleApplication::load() {
  ...
  if (isNotChosen_ == true) {
	...
	if (graph_->loadGraphFromFile(GRAPH_PATH + enterFileName) == 1) {
			 //s21::exitError("Error: cannot read file with graph");
			 std::cout << GREY << "Error: cannot read this graph's file  " << NONE << std::endl;
			 fileName_ = "";
			 delete graph_;
			 delete graphAlg_;
			 isNotChosen_ = true; 
...
void ConsoleApplication::makeDepthFirstSearch(Graph graph, GraphAlgorithms GraphAlgorithms) {
  int *testGraph;
  int start = 1;
...
void ConsoleApplication::makeBreadthFirstSearch(Graph graph, GraphAlgorithms GraphAlgorithms) {
  int *testGraph;
...
void ConsoleApplication::makeDeyxtraAlg(Graph graph, GraphAlgorithms GraphAlgorithms) {
	std::string start;
	std::string finish;
	std::cout << "Enter the first vertex number [1 - "
						<< graph_->getSizeGraph() << "]: ";
	std::cin >> start;
	std::cout << "Enter the second vertex number [1 - "
						<< graph_->getSizeGraph() << "]: ";
	std::cin >> finish;
  int minDex =
      GraphAlgorithms.getShortestPathBetweenVertices(graph, std::stoi(start),
                                            std::stoi(finish));
...
void ConsoleApplication::makeFloydWorshelAlg(Graph graph, GraphAlgorithms GraphAlgorithms) {
  int **matrix;
...
void ConsoleApplication::makeAlgOfPrim(Graph graph, GraphAlgorithms GraphAlgorithms) {
  int **matrix;
...
void ConsoleApplication::makeVoyage(Graph graph, GraphAlgorithms GraphAlgorithms) {
  TsmResult rez;
